...
def calculate_quality_metrics(original, cartoon):
    """Calculate advanced cartoonification accuracy metrics using masked structural alignment."""
    orig_gray = cv2.cvtColor(original, cv2.COLOR_BGR2GRAY)
    cart_gray = cv2.cvtColor(cartoon, cv2.COLOR_BGR2GRAY)

    # 1. EDGE CLEANLINESS (Sparsity Target)
    cart_edges = cv2.Canny(cart_gray, 50, 150)
    edge_density = np.sum(cart_edges > 0) / cart_edges.size
    if 0.02 <= edge_density <= 0.15:
        edge_score = 100.0
    else:
        edge_score = max(0, 100 - abs(edge_density - 0.05) * 1000)

    # 2. TEXTURE REMOVAL (Masked Laplacian Variance)
    # We MUST mask out the black ink lines so they don't mathematically count as "noise"
    mask = cv2.dilate(cart_edges, np.ones((5, 5), np.uint8))
    inv_mask = cv2.bitwise_not(mask)
    
    orig_lap = cv2.Laplacian(orig_gray, cv2.CV_64F)
    cart_lap = cv2.Laplacian(cart_gray, cv2.CV_64F)
    
    orig_var = np.var(orig_lap[inv_mask > 0]) if np.any(inv_mask > 0) else 1.0
    cart_var = np.var(cart_lap[inv_mask > 0]) if np.any(inv_mask > 0) else 0.0
    
    texture_score = min(100, max(0, (1.0 - (cart_var / max(orig_var, 1.0))) * 100))
    texture_score = min(100, texture_score * 1.5) # Boost to reward K-Means flattening

    # 3. COLOR SIMPLIFICATION (Quantization Success)
    orig_colors = len(np.unique(original.reshape(-1, 3), axis=0))
    cart_colors = len(np.unique(cartoon.reshape(-1, 3), axis=0))
    color_score = min(100, max(0, (1.0 - (cart_colors / max(orig_colors, 1.0))) * 100))
    if cart_colors < 150: 
        color_score = max(95.0, color_score)

    # 4. STRUCTURAL INTEGRITY (Dilated Edge Overlap)
    # Checks if the cartoon lines geometrically match the natural contours of the face
    orig_edges = cv2.Canny(orig_gray, 30, 100)
    # Dilate original edges to allow a small margin of alignment error for thick cartoon lines
    orig_edges_dilated = cv2.dilate(orig_edges, np.ones((5, 5), np.uint8))
    
    overlap = cv2.bitwise_and(orig_edges_dilated, cart_edges)
    cart_edge_pixels = np.sum(cart_edges > 0)
    overlap_pixels = np.sum(overlap > 0)
    
    structure_score = (overlap_pixels / max(cart_edge_pixels, 1)) * 100
    # Boost slightly to account for stylistic, connected comic-book lines
    structure_score = min(100, structure_score * 1.3)

    # 5. REGION FLATNESS (Internal Variance)
    cart_smooth = cv2.GaussianBlur(cart_gray, (5, 5), 0)
    flatness_mse = np.mean((cart_gray.astype(float) - cart_smooth.astype(float)) ** 2)
    # Relaxed penalty multiplier so natural gradients aren't severely punished
    flatness_score = min(100, max(0, 100 - flatness_mse * 0.5))  
    ...
    return {
        'texture_removal': texture_score,
        'color_simplification': color_score,
        'region_flatness': flatness_score,
        'edge_enhancement': edge_score,
        'structural_change': structure_score,
